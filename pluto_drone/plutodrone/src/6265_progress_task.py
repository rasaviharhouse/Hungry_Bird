...
class Edrone():
	...
	def __init__(self):
		...
		# [x,y,z,yaw_value]
		self.drone_position = [-3.73,-2.02,31.29,0.0]	

		# [x_setpoint, y_setpoint, z_setpoint, yaw_value_setpoint]
		self.setpoint = [-3.72,-2.02,24.29,0.0] # whycon marker at the position of the dummy given in the scene. Make the whycon marker associated with position_to_hold dummy renderable and make changes accordingly
	...
	def call_ba(self,msg):
		...
		for i in range(0,50):
			...
			#if(abs(self.drone_position[0] - self.setpoint[0]) < 0.4 and abs(self.drone_position[1] - self.setpoint[1]) < 0.4 and abs(self.drone_position[2] - self.setpoint[2]) < 3.0 ):
	...
	#	8. Add error_sum
		...
		self.errorSum[0] += self.error[0]*self.Ki[0] 
		self.errorSum[1] += self.error[1]*self.Ki[1] 
		self.errorSum[2] += self.error[2]*self.Ki[2] 
		self.errorSum[3] += self.error[3]*self.Ki[3] 
		...
		out[0] = (self.Kp[0] * self.error[0]) + (self.errorSum[0]) + (self.Kd[0] * dErr[0])
		out[1] = (self.Kp[1] * self.error[1]) + (self.errorSum[1]) + (self.Kd[1] * dErr[1])
		out[2] = (self.Kp[2] * self.error[2]) + ( self.errorSum[2]) + (self.Kd[2] * dErr[2])
