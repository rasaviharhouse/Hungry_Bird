...
class Edrone():
	...
	def __init__(self):
		...
		# [x,y,z,yaw_value]
		self.drone_position = [-0.86,0.56,55.45,0]	

		# [x_setpoint, y_setpoint, z_setpoint, yaw_value_setpoint]
		self.setpoint = [0.85,-0.5775,37.89,0] # whycon marker at the position of the dummy given in the scene. Make the whycon marker associated with position_to_hold dummy renderable and make changes accordingly
...
if __name__ == '__main__':
	...
	while not rospy.is_shutdown(): 
		e_drone.pid()
		rate.sleep()
		if abs(e_drone.drone_position[0]-e_drone.setpoint[0] <= 1.3) and abs(e_drone.drone_position[1]-e_drone.setpoint[1] <= 1.3) and abs(e_drone.drone_position[2]-e_drone.setpoint[2] <= 1.3) :
			check_drone_flag = 1
		...
		if e_drone.count_no_of_targets == 3 :
			e_drone.disarm()			